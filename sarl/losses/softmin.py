...
class TemperatureScheduler:
    """Schedule temperature beta during training.

    Paper: "anneal beta from 1.0 to 50.0 linearly over the first 50% of training."
    After the annealing phase, beta stays at beta_end.
    """
    def __init__(self, beta_start: float = 1.0, beta_end: float = 50.0,
                 anneal_fraction: float = 0.5, total_epochs: int = 100,
                 schedule: str = 'linear'):
        self.beta_start, self.beta_end = beta_start, beta_end
        self.anneal_fraction = anneal_fraction
        self.total_epochs = total_epochs
        self.anneal_epochs = int(anneal_fraction * total_epochs)
        self.schedule = schedule

    def get_beta(self, epoch: int) -> float:
        if epoch >= self.anneal_epochs:
            return self.beta_end
        progress = epoch / max(1, self.anneal_epochs)
        if self.schedule == 'linear':
            return self.beta_start + progress * (self.beta_end - self.beta_start)
        elif self.schedule == 'exponential':
            log_start = torch.tensor(self.beta_start).log()
            log_end = torch.tensor(self.beta_end).log()
            return (log_start + progress * (log_end - log_start)).exp().item()
        return self.beta_start + progress * (self.beta_end - self.beta_start)
