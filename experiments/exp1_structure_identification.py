    ...
    for structure in structures:
        ...
        for n_samples in sample_sizes:
            ...
            for trial in tqdm(range(n_trials), desc=f"n={n_samples}"):
                ...
                train_loader, _ = create_multi_env_loaders(dataset, dataset, min(126, n_samples // 3), True)
                ...
                # After line 40 in exp1_structure_identification.py, add:
                violations = trainer.objective.compute_violations_only(
                    trainer.model.encode(data.X.to(trainer.device)),
                    data.Y.to(trainer.device),
                    data.E.to(trainer.device)
                )
                print(f"  V1={violations.v1:.4f}, V2={violations.v2:.4f}, V3={violations.v3:.4f}")
